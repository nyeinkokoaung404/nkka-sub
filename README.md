...
- Trilingual (Russian, English, Persian)
...
```
Or remove the following values ​​in `.env' file in `/opt/marzban' folder by deleting their first # from comment mode.
``sh
...
```
In this example, the main language is Farsi.
...
## Versions
To use the host versions, upload the sub folder to the host and change the BASE_URL value to your panel address in the index.php file as in the example below. Remember, if you don't have a certificate for your panel domain, put http as the first word.
```
